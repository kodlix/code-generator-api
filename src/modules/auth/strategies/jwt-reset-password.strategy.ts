...
export class JwtResetPasswordStrategy extends PassportStrategy(
  ...
  async validate(request: Request,{ uuid },
  ): Promise<UserAuthForgottenPasswordEntity> {
    const userForgottenPassword = await this._userAuthForgottenPasswordService.getForgottenPassword({ uuid });
    ...
    const encodedToken = UtilityService.encodeString(token);
